...
The VAE GAN training loop (discriminator construction, alternating G/D updates with AMP scaling and the three-segment LR warmup) is implemented in [`src/ctmr/application/vae_train.py`](../src/ctmr/application/vae_train.py) (`ctmr.application.vae_train`), pinned by `tests/application/test_vae_train.py`.
