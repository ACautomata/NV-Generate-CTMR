...
用法示例（在 sugon DCU 集群上执行）::
  ...
  python -m ctmr.application.acceptance.distribution.synthetic_domain generate \\
      --mode p1 \\
      --case-list /root/private_data/ctmr/l2-synth-eval/case_lists/p1_cases.json \\
      --v1-model-dir /root/private_data/ctmr/models \\
      --output-dir /root/private_data/ctmr/l2-synth-eval/p1_samples

  # Step 2: 生成 P3 式 img2img 样本
  python -m ctmr.application.acceptance.distribution.synthetic_domain generate \\
      --mode p3 \\
      --case-list /root/private_data/ctmr/l2-synth-eval/case_lists/p3_cases.json \\
      --v1-model-dir /root/private_data/ctmr/models \\
      --output-dir /root/private_data/ctmr/l2-synth-eval/p3_samples

  # Step 3: 组装 nnU-Net 输入（把 DM 输出重采样并格式化为仪器输入契约）
  python -m ctmr.application.acceptance.distribution.synthetic_domain prep-inputs \\
      --sample-dir /root/private_data/ctmr/l2-synth-eval/p1_samples \\
      --nnunet-root /root/private_data/ctmr/brats2023_nnunet \\
      --output-dir /root/private_data/ctmr/l2-synth-eval/p1_nnunet_inputs

  # Step 4: 跑冻结仪器推理
  python -m ctmr.application.acceptance.distribution.synthetic_domain predict \\
      --input-dir /root/private_data/ctmr/l2-synth-eval/p1_nnunet_inputs \\
      --results-root /root/private_data/ctmr/nnUNet_results \\
      --output-dir /root/private_data/ctmr/l2-synth-eval/p1_predictions

  # Step 5: 计算指标 + 与真实 R_fail 对照
  python -m ctmr.application.acceptance.distribution.synthetic_domain evaluate \\
      --sample-dir /root/private_data/ctmr/l2-synth-eval/p1_samples \\
      --input-dir /root/private_data/ctmr/l2-synth-eval/p1_nnunet_inputs \\
      --pred-dir /root/private_data/ctmr/l2-synth-eval/p1_predictions \\
      --calibration-summary /root/private_data/ctmr/l2-instrument-calibration/.../metrics \\
      --output-dir /root/private_data/ctmr/l2-synth-eval/report_p1
...
class V1DMSampler:
    ...
    依赖 ``ctmr.infrastructure.maisi_engine.diff_model_infer`` 的模型加载与推理逻辑。
    ...
    def _run_v1_dm_inference(
        ...
        实际执行时，本方法构造 config JSON 并调用 ``ctmr.infrastructure.maisi_engine.diff_model_infer``。
