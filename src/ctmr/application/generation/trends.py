...
"""Shared dev-side trend extractors behind the fixed-cohort watch loops (ticket 09).

The modality-label and mask dev light-acceptance sidecars build their trends
from the same pieces, relocated verbatim from the retired
``scripts/brats_p1_dev_eval.py`` definitions (the cross_modal sidecar uses the
paired PSNR/SSIM chain instead and shares only the shell engine):

- ``DevCohortBuilder``   — the fixed 16-case dev cohort (sha256 order within
                           the per-challenge quotas in ``ctmr.application.shell``);
- ``MrTrendFeatures``    — percentile->RAS->1mm->zero-pad preprocessing plus
                           per-plane RadImageNet slice features (torch.hub);
- ``RealReferenceBank``  — the cached per-modality per-plane dev real features;
- ``TrendFid``           — plane FID points per target modality against the bank;
- ``L2TrendRunner``      — frozen-instrument measurements on the generated
                           pseudo-four-modality cohort (the ADR-0009 single
                           construction point for the predict argv).

Pure stdlib/numpy where possible; torch/nibabel/SimpleITK ride module level
like the pre-move originals.
"""
...
from ctmr.domain.measurement.metrics import FidScoreCalculator
...
class L2TrendRunner:
    ...
    def predict(self, challenge, input_dir, output_dir, log_path):
        ...
        env = {
            **os.environ,
            # the canonical entry runs in a fresh child process: hand it the tree
            # this package was imported from (repo src layout, editable install,
            # or site-packages -- all make ``ctmr.instrument.predict`` importable;
            # a process-local sys.path.insert does not reach the child).
            "PYTHONPATH": os.pathsep.join(
                [str(Path(__import__("ctmr").__file__).resolve().parents[1])] + ([os.environ["PYTHONPATH"]] if os.environ.get("PYTHONPATH") else [])
            ),
            "nnUNet_compile": "f",
