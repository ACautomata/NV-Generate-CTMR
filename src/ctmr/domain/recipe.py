...
Resident in the domain layer since #133 (ADR-0015 M2) -- the values are frozen
domain recipe pins, the shell only invokes them through its first-class hook;
``ctmr.harness.recipe`` keeps a thin re-export for the stage entries.
...
class P3RecipeSpec:
    ...
    def check(self):
        ...
        if self._infer.get("cfg_guidance_scale", 0.0) != self.PINNED_CFG:
            raise ValueError(
                f"P3 candidate is evaluated/selected with CFG OFF (cfg_guidance_scale=0); " f"got {self._infer.get('cfg_guidance_scale')}"
            )
        self._logger.info(
