...
"""Binary-mask morphology on torch tensors (from scripts/utils.py).

Shared ``dilate``/``erode`` wrappers over MONAI's morphological ops; consumed
by ``augmentation`` here and by the mask post-processing still living on its
scripts leaf. Pure tensor math: no IO.
"""
