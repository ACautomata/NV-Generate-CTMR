...
"""Mask-sampling input preparation (from scripts/sample_mask.py, ADR-0015 §2).

Lands the parts that stand alone today: the mask-pipeline input validators
(``check_input_ct`` / ``check_input_mr``, gating ``output_size`` / ``spacing``
/ ``controllable_anatomy_size``) and ``filter_mask_with_organs``.

The DDPM sampler core ``ldm_conditional_sample_one_mask`` stays on its old
leaf until the MAISI engine helpers it consumes (``ReconModel``,
``dynamic_infer``, the mask post-processing) are collected into their own
home; it then completes this module.
"""
