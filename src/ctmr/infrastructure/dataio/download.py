...
"""Hugging Face model/dataset fetch assembly (from scripts/download_model_data.py).

Assembles the per-version file manifest (weights + mask-condition datasets)
and fetches each artifact into its local layout, poking ``config.json`` once
per repo so HF's generic download tracking registers the repo. The old leaf's
argparse front-end belongs to the unified ``ctmr`` CLI layer (ADR-0015 §3)
and does not travel with this module.
"""
