...
while true; do
  out=$(ssh sugon 'n=$(find /root/private_data/l2-synth-eval/p3_samples -name "*.nii.gz" 2>/dev/null | wc -l); alive=$(ps aux | grep img2img_batch | grep -v grep | wc -l); fails=$(grep -h "FAILED" /root/private_data/l2-synth-eval/logs/p3_gpu*.log /root/private_data/l2-synth-eval/logs/p3_shard*.log 2>/dev/null | wc -l); echo "p3=$n alive=$alive fails=$fails"' 2>/dev/null | grep -v WARNING | tail -1)
