...
**L2 肿瘤测量仪器**:
冻结的 **MONAI nnU-Net** BraTS 肿瘤分割网络及其权重，仅用于从生成影像推断 WT/TC/ET mask，以测量肿瘤体积、位置与强化比例分布。五子挑战各持一个专属仪器；它须先在校准集(10% 开发集 ∪ 仪器 fold 内部验证集)的真实 BraTS 上校准误差包络；它不是下游分割效用验证，不以 Dice/HD95 作为生成候选的通过线。
