...
def test_two_bypass_train_steps_match_legacy_loss_params_and_optimizer_state():
    ...
    torch.manual_seed(SEED)
    domain_loss_1 = model.train_step(images, SPACING, MODALITY, executor, controlnet_cond=cond, target_weights=weights)
    domain_loss_2 = model.train_step(images, SPACING, MODALITY, executor, controlnet_cond=cond, target_weights=weights)
...
def test_train_step_guards_the_bypass_and_condition_pairing():
    ...
    with pytest.raises(ValueError, match="training session"):
        bypass_model.train_step(images, SPACING, MODALITY, PlainGradientExecutor(), controlnet_cond=cond)
    ...
    with pytest.raises(ValueError, match="controlnet_cond requires a configured"):
        plain_model.train_step(images, SPACING, MODALITY, PlainGradientExecutor(), controlnet_cond=cond)
