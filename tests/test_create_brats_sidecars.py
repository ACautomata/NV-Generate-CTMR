...
from scripts.create_brats_sidecars import BratsSidecarGenerator
...
@pytest.fixture
def dataset_entries() -> list[LatentEntry]:
    ...
    for case in FAKE_CASES:
        for suffix, modality in sorted(SUFFIX_TO_MODALITY.items()):
            ...
            entries.append(LatentEntry(image=image, modality=modality))
    return entries
...
def embedding_base_dir(
    tmp_path: Path, write_latent: Callable[[Path, str, tuple[float, float, float]], Path], dataset_entries: list[LatentEntry]
) -> Path:
...
@pytest.fixture
def dataset_json(tmp_path: Path, dataset_entries: list[LatentEntry]) -> Path:
...
class TestLatentEntry:
    def test_embedding_relative_path_follows_training_code_convention(self) -> None:
        entry = LatentEntry(image="brats2023-gli/data/case-t1n.nii.gz", modality="mri_t1n")
...
class TestBratsSidecarGenerator:
    def test_writes_one_sidecar_per_training_entry(
        self, build_generator: Callable[[], BratsSidecarGenerator], dataset_entries: list[LatentEntry]
    ) -> None:
        assert build_generator().write_sidecars() == len(dataset_entries)

    def test_sidecar_lies_next_to_latent_with_json_extension(
        self, build_generator: Callable[[], BratsSidecarGenerator], embedding_base_dir: Path, dataset_entries: list[LatentEntry]
    ) -> None:
    ...
    def test_sidecar_spacing_tracks_replaced_latent(
        ...
        dataset_entries: list[LatentEntry],
    ...
    def test_rerun_overwrites_sidecars_to_match_updated_dataset_json(
        self, build_generator: Callable[[], BratsSidecarGenerator], embedding_base_dir: Path, dataset_json: Path, dataset_entries: list[LatentEntry]
    ) -> None:
        build_generator().write_sidecars()
...
class TestCommandLine:
    def test_end_to_end_generation(self, embedding_base_dir: Path, dataset_json: Path, dataset_entries: list[LatentEntry]) -> None:
