...
"""torchrun spawn derivation and assembly routing for the mask train entry (issue #273).

``ctmr generate mask train`` arrives WITHOUT torchrun; the composition root's
train assembly (``ctmr.wiring.generate``, ADR-0019 §2) derives the
``torchrun --nproc_per_node=<num_gpus> -m <module> <argv>`` child (spawn
precedent #123: no fork); already a torchrun worker (``WORLD_SIZE`` set), it
dispatches in-process. These gates demonstrate that derivation for the mask
family: the exact child command and the CLI's WORLD_SIZE fork (the sibling
gates for the modality-label family live in
tests/application/generation/modality_label/test_spawn.py -- basenames stay
...
def test_command_is_the_spawn_torchrun_invocation():
    launcher = TorchrunLauncher(TRAIN_MODULE, ["-e", "env.json", "-c", "config.json", "-t", "net.json"], num_gpus=7)
...
def _fake_train(monkeypatch):
    ...
    """
    import ctmr.application.generation.mask as mask_pkg
    ...
    module.main = _main
    monkeypatch.setattr(mask_pkg, "train", module, raising=False)
...
def test_cli_spawns_torchrun_when_world_size_is_absent(monkeypatch, _fake_train):
    ...
    rc = cli.CtmrCli().run(["generate", "mask", "train", "-e", "env.json", "-g", "4"])
    assert rc == 0
...
def test_cli_dispatches_in_process_when_already_a_torchrun_worker(monkeypatch, _fake_train):
    ...
    rc = cli.CtmrCli().run(["generate", "mask", "train", "-e", "env.json"])
    assert rc == 0
