...
"""Pytest wrapper for the P2 dev-eval selftest (issue #104 / ADR-0013 §2).

Calls the resident ``P2DevEvalSelfTest`` directly (the implementation stays in
...
from scripts.brats_p2_dev_eval import P2DevEvalSelfTest  # noqa: E402  (importorskip must precede the torch-dependent import)


@pytest.mark.torch
def test_p2_dev_eval_selftest(tmp_path):
    failures = P2DevEvalSelfTest(tmp_path).run()
